 ...
 */
export const fontExtensionToFormats = (format) => {
    switch (format) {
        case "woff":
            return "woff";
        case "woff2":
 ...
 */
export const loadFonts = async (web = true) => {
    let typefaceData = [];
    let response;
    return new Promise(async (res, rej) => {
        ...
        for (let i = 0; i < rawTypefaceData.length; i++) {
            ...
            const variants = fileURLs.map(file => { return file.variation; });
            // add to final data
            typefaceData.push({
...
export const searchFontIndex = async (fonts, match) => {
    let left = 0;
...
export const getClosestVariation = (targetVariation, variations) => {
    ...
    if (!variations.includes(targetVariation)) {
        for (dv; !(variations.includes(targetVariation - dv) || variations.includes(targetVariation + dv)); dv += 100)
            ; // find closest match to 400
    }
...
const storeFontDataToLocalStorage = (fontsData, storageKey) => {
 ...
 */
export const beautifiedFontName = {
    "thin": "Thin",
...
export const standardizedFontName = {
    "Thin": "thin",
    "Extralight": "extralight",
    "Light": "light",
    "Regular": "regular",
 ...
 */
export function getFontNameValue(key, mode) {
    if (mode === "name") {
...
export function cleanFiles(files) {
    ...
    function addVariation(newKey) {
        ...
        while (left <= right && result.length !== 0) {
            const middle = Math.floor((left + right) / 2);
            if (result[middle].variation < newKey.variation)
                left = middle + 1;
            else
                right = middle - 1;
        }
    ...
    for (let i = 0; i < fileKeys.length; i++) {
        ...
        addVariation({
            variation: getFontNameValue(key.toLowerCase(), "value"),
            url: files[key]
        });
    }
    return result;
