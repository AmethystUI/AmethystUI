...
*/
const cutil = {
    isEqual: function (...items) {
        // fetch keys from every obj to see if they are equal first
        const root = items[0];
        if (root === undefined || root === null)
            return false; // do not compare undefined values
    ...
    isDefault: function (htmlTag, style, attribute) {
        ...
        if (style[attribute] === undefined)
            return true; // do not generate anything if there is no value
        if (defaultVal === undefined)
            return false; // do not generate anything if there is no value
        if (Object.keys(defaultVal).length === 0)
            return defaultVal === style[attribute]; // non-object comparison
