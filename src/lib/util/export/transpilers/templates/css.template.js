...
export const generateCSSTemplate = (config, elementType, elementStyle, padStyle = true) => {
    const style = padStyle ? _.defaultsDeep(elementStyle, systemDefaultStyles) : elementStyle;
    return genMap(elementType, config, style, !padStyle);
};
const genMap = (tag, conf, style, forceGen = false) => {
    const hmap = new Map();
    const compress = conf.common.compressionAmt > 0;
    // Width and height
    hmap.set("000000x000", () => {
        if (cutil.isDefault(tag, style, "width"))
            return ""; // check default values
        return `width: ${getStringFor.unitedAttr(style.width)};`;
    });
    hmap.set("000001x000", () => {
        if (cutil.isDefault(tag, style, "height"))
            return ""; // check default values
        return `height: ${getStringFor.unitedAttr(style.height)};`;
    });
    hmap.set("000100x010", () => {
        const allMarginAttr = ["marginTop", "marginRight", "marginBottom", "marginLeft"];
        // if literally all default values exist and they ALL equal to the edited margins, don't generate anything
        if (allMarginAttr.every(attr => cutil.isDefault(tag, style, attr)))
            return ""; // check default values
    ...
    hmap.set("000200x000", () => {
        if (cutil.isDefault(tag, style, "paddingTop"))
            return ""; // check default values
        return `padding-top: ${getStringFor.unitedAttr(style.paddingTop)};`;
    });
    hmap.set("000201x000", () => {
        if (cutil.isDefault(tag, style, "paddingRight"))
            return ""; // check default values
        return `padding-right: ${getStringFor.unitedAttr(style.paddingRight)};`;
    });
    hmap.set("000202x000", () => {
        if (cutil.isDefault(tag, style, "paddingBottom"))
            return ""; // check default values
        return `padding-bottom: ${getStringFor.unitedAttr(style.paddingBottom)};`;
    });
    hmap.set("000203x000", () => {
        if (cutil.isDefault(tag, style, "paddingLeft"))
            return ""; // check default values
        return `padding-left: ${getStringFor.unitedAttr(style.paddingLeft)};`;
    });
    hmap.set("010000x000", () => {
        if (cutil.isDefault(tag, style, "opacity"))
            return ""; // check default values
        return `opacity: ${style.opacity}%;`;
    });
    hmap.set("010100x010", () => {
        const allOverflowAttr = ["overflowX", "overflowY"];
        if (allOverflowAttr.every(v => cutil.isDefault(tag, style, v)))
            return ""; // check default values
    ...
    hmap.set("010100x000", () => {
        if (cutil.isDefault(tag, style, "overflowX"))
            return ""; // check default values
        return `overflowX: ${style.overflowX};`;
    });
    hmap.set("010101x000", () => {
        if (cutil.isDefault(tag, style, "overflowY"))
            return ""; // check default values
    ...
    hmap.set("010201x000", () => {
        if (cutil.isDefault(tag, style, "justifyContent"))
            return "";
        return `justify-content: ${style.justifyContent};`;
    });
    hmap.set("010202x000", () => {
        if (cutil.isDefault(tag, style, "alignItems"))
            return "";
        return `align-items: ${style.alignItems};`;
    });
    hmap.set("020000x000", () => {
        if (!style.USEBACKGROUND)
            return null; // do not generate if we're not using a border
        if (cutil.isDefault(tag, style, "backgroundColor"))
            return ""; // check default values
        const clrText = getStringFor.color(style.backgroundColor, conf.common.compressionAmt, conf.stylesheets.colorFmt, conf.stylesheets.colorUnitInference);
        return `background-color: ${clrText};`;
    });
    const USEBORDER = (style.USEBORDER &&
        !cutil.isUnitedValueZero(style.borderWidthTop, style.borderWidthRight, style.borderWidthBottom, style.borderWidthRight) &&
        !cutil.isEqual("hidden", style.borderStyleTop, style.borderStyleRight, style.borderStyleBottom, style.borderStyleLeft)) || forceGen;
    const USEOUTLINE = (style.USEOUTLINE &&
        !cutil.isUnitedValueZero(style.outlineWidth) &&
        style.outlineStyle !== "hidden") || forceGen;
    hmap.set("030000x020", () => {
        if (!USEBORDER || !compress)
            return null; // don't do anything if we're not compressing or not using a border
        const allCheckingAttr = ["borderWidthTop", "borderWidthRight", "borderWidthBottom", "borderWidthLeft", "borderStyleTop", "borderStyleRight", "borderStyleBottom", "borderStyleLeft"];
        ...
        if (!equalStyle || !equalWidth)
            return null;
        const clrText = getStringFor.color(style.borderColor, conf.common.compressionAmt, conf.stylesheets.colorFmt, conf.stylesheets.colorUnitInference);
    ...
    hmap.set("030000x010", () => {
        ...
        const widthAttr = ["borderWidthTop", "borderWidthRight", "borderWidthBottom", "borderWidthLeft"];
        if (widthAttr.every(v => cutil.isDefault(tag, style, v)))
            return ""; // check for default values
    ...
    hmap.set("030001x010", () => {
        if (!USEBORDER || !compress)
            return null; // don't do anything if we're not compressing or not using a border
        // generate border width and styles
        const styleAttr = ["borderStyleTop", "borderStyleRight", "borderStyleBottom", "borderStyleLeft"];
    ...
    hmap.set(compress ? "030002x010" : "030004x000", () => {
        if (!USEBORDER)
            return null; // don't do anything if we're not using a border
        // generate color
        if (cutil.isDefault(tag, style, "borderColor"))
            return ""; // check for default values
        const clr = getStringFor.color(style.borderColor, conf.common.compressionAmt, conf.stylesheets.colorFmt, conf.stylesheets.colorUnitInference);
        return `\nborder-color: ${clr};`;
    });
    hmap.set("030000x000", () => {
        if (!USEBORDER)
    ...
    hmap.set(`${BRchunk}0100x020`, () => {
        if (!(USEBORDER || USEOUTLINE))
            return null; // don't do anything if we're not using a border or not compressing
        const checkDefault = ["borderRadiusTop", "borderRadiusRight", "borderRadiusBottom", "borderRadiusLeft"];
        if (checkDefault.every(v => cutil.isDefault(tag, style, v)))
    ...
    hmap.set(`${BRchunk}0100x010`, () => {
        if (!(USEBORDER || USEOUTLINE))
            return null; // don't do anything if we're not using a border
        if (cutil.isDefault(tag, style, "borderRadiusTop"))
            return ""; // check for default value
        return `border-top-left-radius: ${getStringFor.unitedAttr(style.borderRadiusTop)};`;
    });
    hmap.set(`${BRchunk}0101x010`, () => {
        if (!(USEBORDER || USEOUTLINE))
            return null; // don't do anything if we're not using a border
        if (cutil.isDefault(tag, style, "borderRadiusRight"))
            return ""; // check for default value
        return `\nborder-top-right-radius: ${getStringFor.unitedAttr(style.borderRadiusRight)};`;
    });
    hmap.set(`${BRchunk}0102x010`, () => {
        if (!(USEBORDER || USEOUTLINE))
            return null; // don't do anything if we're not using a border
        if (cutil.isDefault(tag, style, "borderRadiusBottom"))
            return ""; // check for default value
        return `\nborder-bottom-right-radius: ${getStringFor.unitedAttr(style.borderRadiusBottom)};`;
    });
    hmap.set(`${BRchunk}0103x010`, () => {
        if (!(USEBORDER || USEOUTLINE))
            return null; // don't do anything if we're not using a border
        if (cutil.isDefault(tag, style, "borderRadiusLeft"))
            return ""; // check for default value
        return `\nborder-bottom-left-radius: ${getStringFor.unitedAttr(style.borderRadiusLeft)};`;
    });
    hmap.set("040000x010", () => {
        if (!USEOUTLINE || !compress)
            return null; // don't do anything if we're not compressing
        let useableValues = ["outline:"];
        // outline width
        if (!cutil.isDefault(tag, style, "outlineWidth"))
            useableValues.push(getStringFor.unitedAttr(style.outlineWidth));
        // required outline width
        useableValues.push(style.outlineStyle);
        // outline color
        if (!cutil.isDefault(tag, style, "outlineColor")) {
            const clr = getStringFor.color(style.outlineColor, conf.common.compressionAmt, conf.stylesheets.colorFmt, conf.stylesheets.colorUnitInference);
    ...
    hmap.set("040000x000", () => {
        if (!USEOUTLINE)
            return null; // don't do anything if we're not using outline
        if (cutil.isDefault(tag, style, "outlineWidth"))
            return ""; // check for default value
    ...
    hmap.set("040001x000", () => {
        if (!USEOUTLINE)
            return null; // don't do anything if we're not using outline
        if (cutil.isDefault(tag, style, "outlineStyle"))
            return ""; // check for default value
    ...
    hmap.set("040002x000", () => {
        ...
        if (cutil.isDefault(tag, style, "outlineColor"))
            return ""; // check for default value
        return `\noutline-color: ${getStringFor.color(style.outlineColor, conf.common.compressionAmt, conf.stylesheets.colorFmt, conf.stylesheets.colorUnitInference)};`;
    });
    hmap.set("040100x000", () => { return ""; }); // will be radius, if applicable
    hmap.set("040200x000", () => {
        if (!USEOUTLINE)
            return null; // don't do anything if we're not using outline
        if (cutil.isDefault(tag, style, "outlineOffset"))
            return ""; // check default
        return `outline-offset: ${getStringFor.unitedAttr(style.outlineOffset)};`;
    });
    const typeStyle = style.typeStyle ?? null;
    const fontObj = typeStyle?.fontObj ?? null;
    const USETEXT = (style.USETEXT && !!typeStyle && !!fontObj) || forceGen;
    hmap.set("050000x010", () => {
        if (!USETEXT || !compress)
            return null; // don't do anything if we're not using text
    ...
    hmap.set("050100x000", () => {
        if (!USETEXT)
            return null; // don't do anything if we're not using text
        return `\ncolor: ${getStringFor.color(style.color, conf.common.compressionAmt, conf.stylesheets.colorFmt, conf.stylesheets.colorUnitInference)};`;
    });
    const textChunk = compress ? "05" : "06";
    let startLine = compress ? 2 : 0;
    hmap.set(`${textChunk}${hex8(startLine++)}00x100`, () => {
        if (!USETEXT)
            return null; // don't do anything if we're not using text
        if (typeStyle.alignment === "left")
            return ""; // skip if we have no decorations
        return `text-align: ${typeStyle.alignment};`;
    });
    hmap.set(`${textChunk}${hex8(startLine++)}00x100`, () => {
        if (!USETEXT)
            return null; // don't do anything if we're not using text
        if (typeStyle.casing === "none")
            return ""; // skip if we have no decorations
    ...
    hmap.set(`${textChunk}${hex8(startLine++)}00x100`, () => {
        if (!USETEXT)
            return null; // don't do anything if we're not using text
        if (typeStyle.textDecorations.length === 0)
            return ""; // skip if we have no decorations
        const valueMapper = {
            italicize: "",
            underline: "underline",
    ...
    hmap.set(`${textChunk}${hex8(startLine++)}00x100`, () => {
        if (!USETEXT)
            return null; // don't do anything if we're not using text
        if (cutil.isUnitedValueZero(typeStyle.tracking))
            return ""; // skip if we have no decorations
        return `letter-spacing: ${getStringFor.unitedAttr(typeStyle.tracking)};`;
    });
    hmap.set("060000x000", () => { return null; }); // place holder for if chunk 6 was merged with chunk 5
    const USESHADOWS = (style.USESHADOW && style.boxShadows.length > 0) || forceGen;
    hmap.set("070000x000", () => {
        if (!USESHADOWS)
            return null; // do not generate if we're not using shadows
        // this should only run during force generation due to how USESHADOWS is defined
        if (style.boxShadows.length === 0)
        ...
        for (let i = 0; i < style.boxShadows.length; i++) {
            ...
            if (!cutil.isUnitedValueZero(v.radius) || !cutil.isUnitedValueZero(v.grow))
                vStr += getStringFor.unitedAttr(v.radius) + " ";
            if (!cutil.isUnitedValueZero(v.grow))
                vStr += getStringFor.unitedAttr(v.grow) + " ";
            vStr += getStringFor.color(v.color, conf.common.compressionAmt, conf.stylesheets.colorFmt, conf.stylesheets.colorUnitInference);
            shadows.push(vStr);
        }
 ...
 */
const condenseQuadAttr = (top, right, bottom, left, toStr = (v) => `${v}`) => {
