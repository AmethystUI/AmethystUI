<script lang="ts">
    ...
    import { collection, focusedComponent, selectedComponent, selectedOverride, focusedOverride, layerDeleteLock, layerBlurLock } from "$lib/stores/collection";
    ...
    const checkKeyPress = (e:KeyboardEvent):void =>{
        if(e.key === "Enter" || e.key === "Escape"){
            ...
            if (!!$collection[elmntIndex]?.styleOverrides[overrideIndex]) {
                ...
                if($collection[elmntIndex]?.styleOverrides.map(ov => ov.name).includes(name) && name !== $collection[elmntIndex]?.styleOverrides[overrideIndex].name){
